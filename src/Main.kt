...
fun main() {
    ...
    val platosPedidos = sistema.buscarPlatosAlternativo()
...
/**
 * Reemplaza la última ocurrencia de una subcadena en una cadena dada por otra subcadena.
 * @param reemplazado La subcadena que se va a reemplazar.
 * @param reemplazador La subcadena que se va a utilizar como reemplazo.
 * @return La cadena resultante después de reemplazar la última ocurrencia de la subcadena especificada.
 */
fun String.replaceLast(reemplazado: String, reemplazador: String): String {
    // Encuentra la posición de la última ocurrencia de la subcadena a reemplazar, lastIndexOf devolvera -1 si no encuentra ninguna ocurrencia
    val posicionUltima = this.lastIndexOf(reemplazado)

    // Si no se encuentra la subcadena a reemplazar, devuelve la cadena original sin cambios
    if (posicionUltima == -1) {
        return this
    } else {
        return this.substring(0, posicionUltima) + reemplazador + this.substring(posicionUltima + reemplazado.length) //sumamos reemplazado.length para tomar la substring que empieza donde acaba el valor a reemplazar
    }
}